...
against a reference simulation, via :mod:`bemyself.turing`) and
``cyc(t1,t2,d)`` (a translation-cycle certificate, checked through the
existing ``[CYCLE]`` machinery of :mod:`bemyself.claimtypes.cycle`).
...
from bemyself.checks import DEFAULT_HALT_LIMIT
from bemyself.model import Claim, Verdict
from bemyself.msheet import formula
from bemyself.msheet.formula import Def, EmptyRangeError, EvaluationError, FormulaError, UnboundedError
...
# (bemyself/msheet/README.md, "Einen neuen Zeugen-Typ hinzufuegen").
