...
```
python3 -m bemyself.msheet run tests/data/beispielblatt.msheet
python3 -m bemyself.msheet run blatt.msheet --json --sandbox require
```
...
| `sim(a..b)` | Checkpoints des Ziels (`cp t: (Q,p,T)`) gegen die Referenz-Simulation (`bemyself.turing`). Der Checkpoint-Tape steht für das **beschriebene Fenster** (von der ersten links- bis zur letzten rechts-beschriebenen Zelle; Kopfposition absolut). |
...
from bemyself.msheet.witnesses import WitnessKind, WitnessResult, WitnessSpec
