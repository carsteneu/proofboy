...
``proofboy eval --set <datei>`` rebuilds the fixture the set was generated
...
from proofboy import evalset, profiles
from proofboy.checks import DEFAULT_COMMAND_ALLOWLIST, Ctx, run_claim
from proofboy.cli import EXIT_ERROR, EXIT_OK, EXIT_STRICT, exit_code, sanitize
from proofboy.claimtypes.coloring import DEFAULT_COLORING_LIMIT
from proofboy.claimtypes.compute import DEFAULT_COMPUTE_ALLOWLIST
from proofboy.claimtypes.cycle import DEFAULT_CYCLE_LIMIT
from proofboy.claimtypes.halt import DEFAULT_HALT_LIMIT
from proofboy.claimtypes.search import DEFAULT_SEARCH_LIMIT
...
def _fail(args, message):
    ...
    print(f"proofboy: {message}", file=sys.stderr)
    if args.json:
        print(
            json.dumps(
...
def run_eval(args):
    """CLI entry point for ``proofboy eval``."""
    ...
    if os.path.isdir(fixture_root) and not os.path.exists(marker):
        ...
        return _fail(
            args,
            f"refusing to delete {fixture_root}: not created by proofboy eval; "
    ...
    try:
        ...
        with open(marker, "w", encoding="utf-8") as handle:
            handle.write("fixture of proofboy eval; safe to delete\n")
    ...
    if anchors["base"] != fixture.commits["base"] or anchors["head"] != fixture.commits["fixed"]:
        return _fail(
            ...
            "regenerate it with: python3 -m proofboy.evalset <out.json>",
        )
    report = evaluate(
        document,
        fixture,
