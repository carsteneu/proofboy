...
from proofboy import claimtypes, profiles, projectconfig, stackdetect, toolmanifest
from proofboy.model import Cause, Claim, Verdict
from proofboy.report import parse_report
from proofboy.scratchpad import DEFAULT_DB, ScratchpadError, default_db_path, read_section
...
def list_types(json_mode=False):
    ...
    Read from the registry (``proofboy.claimtypes.CLAIM_TYPES``), never a
...
def run_check(args, parser):
    ...
    if not claims:
        ...
        print(f"proofboy: no verifiable claims found in the {kind}", file=sys.stderr)
        if args.json:
...
def main(argv=None):
    ...
    if args.command == "eval":
        ...
        from proofboy.eval import run_eval
