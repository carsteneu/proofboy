...
- the markers of every registered optional claim type
  (:data:`proofboy.claimtypes.CLAIM_TYPES`), for example ``[HALT: ...]``
...
(:func:`proofboy.claimtypes.marker_tokens`); a template body keeps the
...
_MAX_LINE = 8192
# The shape of a commit hash; proofboy/checks.py validates the same shape.
...
# proofboy/claimtypes/halt.py for the same shape).
...
# advance at most to the next delimiter (see proofboy/claimtypes/halt.py for
# the same reasoning).
