"""Deterministic fixture and evaluation set for ``python3 -m proofboy eval``.
...
from proofboy import profiles
from proofboy.cli import MAX_REPORT_BYTES
from proofboy.model import Cause
...
def build_fixture(root):
    ...
    # command (python3 -m proofboy.turing), in its own commit: the false
    ...
    os.makedirs(os.path.join(repo, "proofboy"), exist_ok=True)
    _write(repo, os.path.join("proofboy", "turing.py"), _FIXTURE_TURING)
    ...
    os.makedirs(os.path.join(repo, "proofboy", "experiments"), exist_ok=True)
    _write(
        repo,
        os.path.join("proofboy", "experiments", "erdos_straus.py"),
        _FIXTURE_ERDOS_STRAUS,
    )
    ...
    cases = [
        ...
        case(
            ...
            "gibt aber 'fixture' aus).",
                ...
                f"[COMPUTE: python3 -m proofboy.turing 1RB1RZ_0LA0LA 3 -> {'0' * 64}]",
            ),
            targets=["compute"],
        ...
        case(
            ...
            "python3 -m proofboy.experiments.erdos_straus auf dem Fixture-Stub "
            ...
            "ihn bliebe der Lauf unpruefbar statt CONFIRMED.",
                ...
                "[COMPUTE: python3 -m proofboy.experiments.erdos_straus 8 -> "
        ...
        case(
            ...
            "zaehlen.",
                ...
                # untruthful scope.
                "**Files in scope:** proofboy/model.py, <pfad2>",
...
def _main(argv):
    if len(argv) != 2:
        print("usage: python3 -m proofboy.evalset <out.json>", file=sys.stderr)
        return 2
    out_path = os.path.abspath(argv[1])
    with tempfile.TemporaryDirectory(prefix="proofboy-evalset-") as work:
