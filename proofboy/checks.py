...
Every checker returns a :class:`~proofboy.model.Result` carrying the verdict
...
def check_branch_pushed(claim: Claim, ctx: Ctx) -> Result:
    ...
    private_ref = f"refs/proofboy-verify/{os.urandom(8).hex()}"
    fetch_args = [
        "-c",
        "core.sshCommand=ssh",
        "-c",
...
def check_unknown_marker(claim: Claim, ctx: Ctx) -> Result:
    ...
    The report parser (proofboy/report.py) emits this claim; nothing about it
...
def check_profile(claim: Claim, ctx: Ctx) -> Result:
    ...
    The profile module (proofboy/profiles.py) emits this claim when the
...
def run_claim(claim: Claim, ctx: Ctx, registry: dict | None = None) -> Result:
    ...
    (:mod:`proofboy.claimtypes`). A hostile report must never crash the
