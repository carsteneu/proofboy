...
4. **Axiom query.** The checker's own program (``proofboy/tools/lean_axioms.lean``)
...
from proofboy import toolmanifest
from proofboy.claimtypes.halt import _ARROW, _UNICODE_ARROW
...
# (same shape as [COMPUTE], see proofboy/claimtypes/halt.py for the reasoning).
...
_ANSWER_RE = re.compile(
    r"\APROOFBOY-LEAN-AXIOMS (?P<name>[^\s]+) \[(?P<axioms>[^\]\n]*)\]\s*\Z"
)
_UNKNOWN_RE = re.compile(r"\APROOFBOY-LEAN-UNKNOWN (?P<name>[^\s]+)\s*\Z")
_FOREIGN_RE = re.compile(r"\APROOFBOY-LEAN-FOREIGN (?P<name>[^\s]+) (?P<module>[^\s]+)\s*\Z")
_QUERY_ERROR_RE = re.compile(r"\APROOFBOY-LEAN-ERROR (?P<detail>.*)\Z")
