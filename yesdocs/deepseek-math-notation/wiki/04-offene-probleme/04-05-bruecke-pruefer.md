...
Die Verdicts im Detail [ebd.](git:yesloop/proofboy-p7-halt:proofboy/claimtypes/halt.py, accessed 2026-09-12):
...
Neue Claim-Typen sind als Erweiterungspunkt vorgesehen: ein Modul im Paket `proofboy/claimtypes/` plus ein Eintrag in `CLAIM_TYPES`; Report-Parser (`proofboy/report.py`) und Checker-Dispatcher (`proofboy/checks.py`) konsultieren die Registry und brauchen keine Änderung [claimtypes/__init__.py (lokale Quelle)](git:yesloop/proofboy-p7-halt:proofboy/claimtypes/__init__.py, accessed 2026-09-12). Der Standard-Evalsatz (`proofboy/evalset.py`, `tests/data/pruefset.json`) enthält vier HALT-Fälle im ansonsten git-/test-basierten 34-Fall-Set [evalset.py (lokale Quelle)](git:yesloop/proofboy-p7-halt:proofboy/evalset.py, accessed 2026-09-12); die Fixture ist deterministisch gebaut (feste Hashes), der Lauf benötigt kein Netz [ebd.](git:yesloop/proofboy-p7-halt:proofboy/evalset.py, accessed 2026-09-12).

## 3. Was für den Testplan bereits steht

1. **Ein vollständiger Prüfpfad für Halte-Behauptungen**: Marker → Parser → Registry → Simulator → Verdict, in-process, ohne Netz [report.py (lokale Quelle)](git:yesloop/proofboy-p7-halt:proofboy/report.py, accessed 2026-09-12).
2. **Score-Prüfung** in derselben Zeile — deckt die BB-typische Doppelbehauptung (Schritte *und* Einsen) ab [halt.py](git:yesloop/proofboy-p7-halt:proofboy/claimtypes/halt.py, accessed 2026-09-12).
3. **Kalibrierte Referenzwerte** (BB(5)-Champion etc.) als Ground Truth [tests/test_turing.py](git:yesloop/proofboy-p7-halt:tests/test_turing.py, accessed 2026-09-12).
4. **Untrusted-Input-Härtung** (lineare Muster, int-Digit-Cap, Limit) — ein Testfeld kann mit feindlichen/übergroßen Modellantworten umgehen [halt.py](git:yesloop/proofboy-p7-halt:proofboy/claimtypes/halt.py, accessed 2026-09-12).
5. **Drei-Verdict-Disziplin** aus der Doktrin, im Code verankert [checks.py](git:yesloop/proofboy-p7-halt:proofboy/checks.py, accessed 2026-09-12) [falsifizierbarkeit.md (lokale Quelle)](../../../pruefer/wiki/01-theorie/falsifizierbarkeit.md, accessed 2026-09-12).
...
5. **Score-only-Claims fehlen** — `[SCORE]` ohne `[HALT]` ist ausdrücklich keine Behauptung; Aufgaben, die nur den Score abfragen, sind nicht abbildbar [halt.py](git:yesloop/proofboy-p7-halt:proofboy/claimtypes/halt.py, accessed 2026-09-12).
6. **Report-Budget/Timeouts fehlen.** Das Claim-Limit ist pro Claim; ein Bericht mit vielen langen Claims kann Minuten binden („one claim at the limit means roughly five seconds") [ebd.](git:yesloop/proofboy-p7-halt:proofboy/claimtypes/halt.py, accessed 2026-09-12). Für A/B-Läufe mit hunderten Aufgaben braucht der Harness (05-04) ein Gesamtbudget.
...
8. **Visualisierung/Logging für A/B.** Weder Token-Zählung noch Trial-Logs sind Teil des Prüfer-Scopes; der Test-Harness muss sie selbst mitbringen (Grenze zieht `evalset.py`, das nur deterministische Fixtures kennt) [evalset.py](git:yesloop/proofboy-p7-halt:proofboy/evalset.py, accessed 2026-09-12).
...
**Lokale Quellen** (Branch `yesloop/proofboy-p7-halt`; gemerged in lokalen `master` als `4fd2b30`, Zugriff 2026-09-12)
1. `proofboy/claimtypes/halt.py` — [HALT]/[SCORE]-Claim-Typ, Verdicts, Limit, Härtung.
2. `proofboy/turing.py` — unabhängiger TM-Simulator (parse/run), bbchallenge-Notation, Semantik.
3. `tests/test_turing.py` — Hand-Traces und publizierte Referenzmaschinen (BB(5)-Champion etc.).
4. `proofboy/claimtypes/__init__.py` — Claim-Typ-Registry und Erweiterungsrezept.
5. `proofboy/report.py` — Marker-Parsing, lineare Muster.
6. `proofboy/checks.py` — Checker-Dispatcher, Verdict-Disziplin, Allowlist.
7. `proofboy/evalset.py` + `tests/data/pruefset.json` — deterministische Fixture, 34-Fall-Set.
