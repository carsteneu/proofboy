...
|---|---|---|
| Lean-Mandat-Runner | `tooling/lean_mandat.py` (neu) | Matrix-Runner analog [05-14](05-14-v1-1-lean-v18.md) §2 (`lean_smoke`): fünf Varianten (**V-A** Regeln als ``--``-Kommentare, Systemprompt selbst Lean; **V-B** + ein Denkspur-Beispiel; **V-C** + drei Beispiele; **V-D** eigener Systemprompt `SYS_D` + Completion-Prefill im Assistant-Content; **V-E** = V-B + Fence-Verbot) × sieben Aufgaben der Grade trivial/mechanisch/Lemma × Wiederholungen. Direkter Modellaufruf über `harness.call_model` (kein Arm, kein Sockel), Extraktion wie `lean_smoke` (größter Zaun-Block, sonst Volltext) mit Fence-Flag, Prefill-Echo-Erkennung, Statement-Echo, RC-Metriken via `leanfidelity`, Elaboration via `leancheck` (inkl. Axiom-Sonde `main_thm`), deterministische Renderer (JSON + zwei Markdown-Assets) und `render`-Subkommando. |
...
V-D hängt den unfertigen Beweis als Assistant-Content an (inline-Verhalten, **ohne** `prefix: true`; der Beta-Prefix-Modus der Doku [DeepSeek 2026] blieb bewusst ungetestet — eine Grenze, §5.4). Zwei Konsequenzen sind im Werkzeug abgebildet: (1) Der `content` kann den Prefill wiederholen — Feld `prefill_echo`, `continuation` = eigener Text; in der Hauptmatrix trat das **nicht** auf (0/21 — das Modell emittierte jeweils einen frischen Komplettbeweis statt der Fortsetzung), im Smoke-Lauf einmal (1/1: Prefill + `decide\n  exact h1`); (2) der Prefill trägt den Theorem-Vorspann (`import Std`/`theorem main_thm … := by have h1 : … := by `) — anders als inline, wo der Prefill nackt war und damit a priori nicht elaborierbar. **Wichtig für die Lesart:** V-D nutzt zudem — inline-treu — einen eigenen, kürzeren Systemprompt (`SYS_D`: Regeln als Kommentare mit Beispiel) statt der SYS_A-Basis der übrigen Varianten. V-D ist damit eine *Konfiguration* aus geschärftem Prompt und Prefill; die 0-Fence-Quote (§5.2) belegt nicht den Prefill allein.

### 3.4 Vierte Messgröße: der Augenschein

Die RC-Heuristik (`leanfidelity`) klassifiziert Zeilen per erstem Wort und `:=`-Bindung. Schon der Live-Smoke zeigte beide Fehlerrichtungen: Prosa, die Lean zitiert („theorem main_thm : … := by …"), zählt als Statement-Zeile; Lean-Syntaxfortsetzungen (`| zero =>`) zählen als Prosa. Deshalb misst die Runde zusätzlich per **Augenschein-Stichprobe** (14 Zellen — V-A: t1/m1/l1/l3, V-B: t1/m1/l1/l3, V-C/V-D/V-E: je t1/l1 —, ~13 % der 105) den Anteil „echter Lean-Zeilen" (Zeilen, die als Lean parsen würden) und stellt beide Zahlen nebeneinander (§5.1).
...
| V-D (SYS_D + Prefill) | 21 | **21** | 0 | 0 | **0** | 21 | 0,5170 |
...
**Zuverlässigste Konfiguration der Runde:** **V-D** (eigener Systemprompt `SYS_D` + Completion-Prefill) 21/21 valid, 0 Fence-Lecks, Statement-Echo 21/21 — die V-D-Konfiguration macht den sichtbaren Kanal durchgehend Lean-förmig. Zwei Einschränkungen der Lesart: (a) Der Prefill wurde in der Hauptmatrix **nicht fortgesetzt** — `prefill_echo` 0/21, das Modell emittierte jeweils einen frischen Komplettbeweis (der Echo-Beleg stammt aus dem Smoke-Lauf, 1/1); (b) V-D hat zugleich einen eigenen, geschärften Systemprompt (§3.3). Belegt ist also die *Konfiguration*, nicht der Prefill allein. V-B 21/21 valid, aber 9 Fence-Lecks — der sichtbare Kanal bleibt valide, die Konvention „pures Lean" hält dort nicht gegen das Fence-Verbot.
...
5. **Test-Inventar.** Die Runde hebt die Suite auf 1170 Tests (+29; Basis 1141); `make test` bleibt der Eingang.
