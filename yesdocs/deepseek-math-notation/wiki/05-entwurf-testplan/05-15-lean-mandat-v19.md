...
| Tests | `tests/test_v19_lean_mandat.py` (neu, 26) | Varianten-Ladder (V-A ⊂ V-B ⊂ V-C; V-E = V-B + Fence-Zusatz; nur V-D mit Prefill), Prompt-Bau, Extraktion/Echo/Statement-Echo, Aggregation, geskripteter Transport und geskriptetes `leancheck` (kein Netz), Renderer-Determinismus (zweimal rendern ⇒ identisch), echte Referenz-Elaboration (skipWithout Toolchain). Volle Suite: **1167 Tests grün** auf dieser Basis; Basis (8832f9f): 1141 grün, `diff` der Fehlschläge: keine. |
...
**Systematisch:** 105 Zellen, kein Transportfehler. **94 valid · 6 invalid · 5 ohne Code** (Budget, s. o.); `sorry` 0×; unter den 94 validen sind **88 axiomfrei** (5 × `propext`/`Quot.sound`, 1 × `native_decide`-Axiom; die Axiom-Sonde der 2 invaliden Zellen bestand separat, wird hier aber nicht mitgezählt). **Statement-Echo 100/105** (jede Antwort mit Inhalt trägt die geforderte Aussage, nur die 5 Budget-Leerzellen nicht). **Fence-Rate 21/105 = 20 %** trotz expliziter Verbote: V-A 3/21, V-B **9/21**, V-C 6/21, V-D **0/21**, V-E 3/21 (der Fence-Zusatz halbiert das Leck gegenüber V-B, beseitigt es nicht; nur der Prefill ist fence-frei).
...
5. **Test-Inventar.** Die Runde hebt die Suite auf 1167 Tests (+26; Basis 1141); `make test` bleibt der Eingang.
