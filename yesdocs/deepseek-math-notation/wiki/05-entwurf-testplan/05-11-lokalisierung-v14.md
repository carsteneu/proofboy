...
7. **Aus der Review-Runde (alle niedrig, teils behoben):** behoben — ReDoS-Muster im Scan ersetzt (lineare Rückwärtssuche statt verankertem Regex; Regressionstest), `--runs-root` außerhalb des Repos crasht nicht mehr (Anzeige-Helfer), `cmd_one`-Logpfad trägt das Level, fehlender `--runs`-Pfad des Scans ist jetzt ein Fehler statt „0 gescannt“, ebenso ein leerer/falscher `--sets`-Pfad in Scan und Auswertung (sonst sähe ein Operator-Fehler wie ein sauberer Befund aus), Klassen-/Leg-Doppelzählung dokumentiert, Docstrings/Kommentare an Code und Checker angeglichen (§5.2-Wortlaut korrigiert: Wertwechsel auf andere *gültige* Zertifikate). Offen (bewusst): `binding_wrong`-Note nur bei leerer Notizenliste (eine Runde Verzögerung möglich), Bindungs-Metrik ohne Arm-Filter (strukturell auch für K/B denkbar; in den Daten 0 Fälle), lexikografische Set-Sortierung bei zweistelligen Minors (`0.10`), Scan blind für alternative Wert-Schreibweisen (führende Null, gesplittet, Unicode-Minus) — für die aktuellen digit-freien Texte ohne Wirkung, für künftige Leck-Bugs als Fehlalarm-Kandidaten bewusst konservativ. Zusätzlich (Vorbestand, nicht angefasst): `evaluate` meldet bei fehlendem `--runs`-Pfad eine leere Tabelle statt eines Fehlers (alte `load_runs`-Semantik).
