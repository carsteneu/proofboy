...
**Unendliche Behauptungen** (`forall n in N: …`) sind auf dieser Maschine **nicht ausführbar**: Der Runner vergibt `UNVERIFIABLE` mit Grund `unbounded_claim` — es sei denn, das Blatt liefert für eine formel-reine Teilklasse einen Referenzbeweis (Lean/Metamath; siehe [05-03](05-03-mapping-formal.md), dort auch die Grenze: keine der beiden Toolchains ist auf dieser Maschine installiert). Diese Ehrlichkeit ist Teil des Designs: `UNVERIFIABLE` ist ein ehrlicher Zustand, kein Fehler — genau wie im Vorbild `bemyself eval`, das eine `Unpruefbar-Quote` berichtet statt sie zu verstecken (lokale Quelle: `bemyself/eval.py`, gelesen 2026-09-12).
