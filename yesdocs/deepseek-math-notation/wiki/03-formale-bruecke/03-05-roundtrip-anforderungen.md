...
**Schnittstelle [CYCLE]-Check ↔ Theorem-Hypothesen.** Der Check vergleicht die Konfigurationen bei `t1` und `t2` nur auf den erreichbaren Zellen (Lin-Fenster; `bemyself/claimtypes/cycle.py`, lokale Quelle); der Satz nimmt die volle Translations-Gleichheit der Konfiguration an. Für Läufe auf dem leeren Band (die beiden formalisierten Maschinen) fallen beide Bedingungen zusammen — dort ist die Zelle geschlossen. Die Differenz ist die benannte Grenze zwischen Check und Theorem, kein Formfehler: Zellen außerhalb des Fensters können abweichen, ohne die Nicht-Halt-Aussage zu verletzen.
