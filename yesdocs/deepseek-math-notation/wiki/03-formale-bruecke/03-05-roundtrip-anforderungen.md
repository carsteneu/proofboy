...
Für die Python-Zeugenstufe tritt an die Stelle des Audits die Verdikt-Disziplin: „a verdict is `CONFIRMED` only when the check actually ran and proved the claim, and a claim that cannot be run is `UNVERIFIABLE` — never `CONFIRMED`" (`bemyself/checks.py`, lokale Quelle, Branch `yesloop/bemyself-p7-halt`, gelesen 2026-09-12).
...
| Zeugen-Checker | ja, klein | deterministisch, unabhängig reimplementiert (`bemyself/turing.py`, lokale Quelle) |
...
**Maschinen-Realität.** Die Feststellung „kein `lean`/`lake`" ist überholt: Lean 4.33.1 (via elan) ist seit 2026-09-12 auf dieser Maschine installiert, und die Lean-Spalte ist für die hier formalisierte Zelle ausgeführt statt doku-basiert. Das relativiert die Momentaufnahmen in §4 (Hinweis bei T3) und in `05-03` §5/§7 — die normativen Anforderungen bleiben unverändert. Ausgeführt sind bisher der Kernel-Beweis und der Axiom-Audit (`#print axioms`, T6-Muster); die Roundtrip-Tests T2/T3 auf dem Lean-Zweig bleiben offen.
